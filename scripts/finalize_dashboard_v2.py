...
def replace(path, old, new, count=1):
    ...
    s = p.read_text()
    if old not in s:
        raise SystemExit(f"patch target not found in {path}: {old[:80]}")
    p.write_text(s.replace(old, new, count))


# Keep RT-02 user-visible semantics synchronized with the Candidate-v9 terminal.
replace(
    "src/i18n.jsx",
...
candidate9 = """  'pda-candidate-v9-development-fail': {
...
"""
replace("src/i18n.jsx", anchor, candidate9 + anchor)
replace(
    "src/i18n.jsx",
...
# Compile a real dependency DAG instead of leaving human-readable dependency text only.
compiler = Path("scripts/compile-dashboard-state.mjs")
...
"""
if marker not in s:
    raise SystemExit("compiler DAG insertion target not found")
compiler.write_text(s.replace(marker, "\n" + dag + marker, 1))

# Highlight the actual prerequisite chain for the current critical-path node.
roadmap = Path("src/components/NorthStarRoadmap.jsx")
s = roadmap.read_text()
old = "criticalPath: 'Jump to current critical path',"
if old not in s:
    raise SystemExit("English critical path copy target not found")
s = s.replace(old, "criticalPath: 'Jump to current critical path', criticalNode: 'Critical path',", 1)
old = "criticalPath: '跳到目前關鍵路徑',"
if old not in s:
    raise SystemExit("Chinese critical path copy target not found")
s = s.replace(old, "criticalPath: '跳到目前關鍵路徑', criticalNode: '關鍵路徑',", 1)
old = "function TimelineNode({ node, index, t, locale, copy, expanded, onToggle }) {"
if old not in s:
    raise SystemExit("TimelineNode signature target not found")
s = s.replace(old, "function TimelineNode({ node, index, t, locale, copy, expanded, onToggle, critical }) {", 1)
old = '<article className={`roadmap-node status-${node.status}`} id={`roadmap-node-${node.id}`}>'
if old not in s:
    raise SystemExit("TimelineNode class target not found")
s = s.replace(old, '<article className={`roadmap-node status-${node.status} ${critical ? \'is-critical-path\' : \'\'}`} id={`roadmap-node-${node.id}`}>', 1)
old = '<div className="roadmap-node-kicker"><span>{node.track}</span>{node.requiredForMvj ? <span>{t(\'timeline.mvjRequired\')}</span> : null}</div>'
if old not in s:
    raise SystemExit("TimelineNode kicker target not found")
s = s.replace(old, '<div className="roadmap-node-kicker"><span>{node.track}</span>{critical ? <span>{copy.criticalNode}</span> : null}{node.requiredForMvj ? <span>{t(\'timeline.mvjRequired\')}</span> : null}</div>', 1)
stats_marker = "  const nodeMatches = (node) => {\n"
critical_calc = """  const criticalPathIds = useMemo(() => {
...
"""
if stats_marker not in s:
    raise SystemExit("critical-path calculation target not found")
s = s.replace(stats_marker, critical_calc + stats_marker, 1)
old_map = '<TimelineNode key={node.id} node={node} index={index} t={t} locale={locale} copy={copy} expanded={expandedNodes.has(node.id)} onToggle={toggleNode} />'
new_map = '<TimelineNode key={node.id} node={node} index={index} t={t} locale={locale} copy={copy} expanded={expandedNodes.has(node.id)} onToggle={toggleNode} critical={criticalPathIds.has(node.id)} />'
if old_map not in s:
    raise SystemExit("critical-path TimelineNode target not found")
roadmap.write_text(s.replace(old_map, new_map, 1))

css = Path("src/control-plane.css")
s = css.read_text()
needle = "  .live-state{font-size:.76rem;white-space:nowrap}"
if needle not in s:
    raise SystemExit("critical path CSS target not found")
css.write_text(s.replace(needle, "  .roadmap-node.is-critical-path .roadmap-node-card{box-shadow:inset 3px 0 0 currentColor}\n" + needle, 1))

# Strengthen CI parity validation across localized verified surfaces.
validator = Path("scripts/validate-dashboard-state.mjs")
s = validator.read_text()
old = "const i18n = await readJson('dashboard/state/i18n/control-plane.json');"
if old not in s:
    raise SystemExit("validator i18n source target not found")
s = s.replace(old, old + "\nconst i18nSource = await readFile(resolve('src/i18n.jsx'), 'utf8');", 1)
parity_anchor = "assert(Object.keys(i18n['zh-Hant']).sort().join('|') === Object.keys(i18n.en).sort().join('|'), 'zh/en control-plane keys differ');"
parity_block = """assert(Object.keys(i18n['zh-Hant']).sort().join('|') === Object.keys(i18n.en).sort().join('|'), 'zh/en control-plane keys differ');
...
if parity_anchor not in s:
    raise SystemExit("parity validator target not found")
validator.write_text(s.replace(parity_anchor, parity_block, 1))

# Unit-test the critical structural dependency chain as a regression guard.
tests = Path("tests/state.test.mjs")
s = tests.read_text()
addition = """
...
"""
if "critical path dependency chain remains acyclic" not in s:
    tests.write_text(s.rstrip() + "\n" + addition)

print("final Dashboard v2 patch staged")
